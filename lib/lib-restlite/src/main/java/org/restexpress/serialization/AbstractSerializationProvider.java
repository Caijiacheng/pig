...
implements SerializationProvider
{
	...
	public SerializationSettings resolveRequest(Request request)
	{
		...
		if (processor == null)
		{
			List<MediaRange> requestedMediaRanges = MediaTypeParser.parse(request.getHeader(HttpHeaders.Names.CONTENT_TYPE.toString()));
		...
		return new SerializationSettings(request.getHeader(HttpHeaders.Names.CONTENT_TYPE.toString()), processor);
	}
    ...
    public SerializationSettings resolveResponse(Request request, Response response, boolean shouldForce)
    {
		...
		if (processor == null)
		{
			List<MediaRange> requestedMediaRanges = MediaTypeParser.parse(request.getHeader(HttpHeaders.Names.ACCEPT.toString()));
