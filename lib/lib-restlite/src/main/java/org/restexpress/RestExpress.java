...
public class RestExpress
{
	...
	public void bind()
	{
		bind((getPort() > 0 ? getPort() : DEFAULT_PORT));
	}
...
//			bootstrap = Bootstraps.createServerNioBootstrap();
		...
		{
			IOGroup = new NioEventLoopGroup(getIoThreadCount());
		}
...
//		bootstrap.setOption("receiveBufferSize", getReceiveBufferSize());
		...
		bootstrap.childOption(ChannelOption.SO_KEEPALIVE, serverSettings.isKeepAlive());
		...
		bootstrap.option(ChannelOption.MAX_MESSAGES_PER_READ, getReceiveBufferSize());
	}
