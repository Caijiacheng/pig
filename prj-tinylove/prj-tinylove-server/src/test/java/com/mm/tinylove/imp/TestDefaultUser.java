...
public class TestDefaultUser {
	...
	public void testUserCheckConsist() {
				...
				IMessage imsg = user.publishMsg(pair, content, location,
						imgurl, null);
				...
				Assert.assertEquals(user.userStorys().all().get(0).message()
						.size(), 1);
				IMessage iimsg = user.userStorys().all().get(0).message().all()
						.get(0);
	...
	public void testBuildStory() {
				...
				for (int i = 0; i < message_num; i++) {
					user.publishMsg(pair, content + ":" + i, location, imgurl,
							null);
					user.publishMsg(pair1, content + ":" + i, location, imgurl,
							null);
				}
				...
				Assert.assertEquals(user.userStorys().all().get(0).message()
						.size(), message_num);
				Assert.assertEquals(user.userStorys().all().get(1).message()
						.size(), message_num);
	...
	public void testPublishComments() {
				...
				IUser user = UserStorage.createUserAndSave();
				IPair pair = user.createPair("tt");
				IMessage msg = user.publishMsg(pair, content, location, imgurl,
						null);
				int user_num = 100;

				for (int i = 0; i < user_num; i++) {
					IUser user1 = UserStorage.createUserAndSave();
					IComment comment = user1.publishComment(msg,
							"llll oooo vvvv ittttt:" + i);
