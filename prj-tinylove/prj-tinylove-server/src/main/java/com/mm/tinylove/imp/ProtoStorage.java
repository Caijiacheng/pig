 ...
 * 
 * 当然，可能会有些数据类型会打破这个规则，比如说用户信息IUser 1）由于这个更新的内容不是敏感的，所以，完全可以扔到缓存中，等缓存中的数据失效了重新加载
 * 
 * 
 * QA: FIXME: 1. Cache support CopyOnWrite()?
 * 
 * @author caijiacheng
 * 
 * @param <T>
...
public class ProtoStorage<T extends Message> extends KVStorage {
	...
	public void unmarshalValue(byte[] data) {
		try {
			value = (T) newBuilder().mergeFrom(data).build();
			immutable = true;
	...
	public boolean equals(Object obj) {
		if (obj instanceof ProtoStorage<?>) {
			ProtoStorage<?> o = (ProtoStorage<?>) obj;
			return Objects.equals(o.getKey(), getKey());
		}
